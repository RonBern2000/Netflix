...
import { REDIS_URI , NODE_ENV, REDIS_PORT, PORT, API_READ_ACCESS_TOKEN, API_KEY, RABBITMQ_URL, PROXY_URL } from "./config/env";
import { InvalidEnvironmentVariablesError } from '@netflix-utils/shared/build/errors/invalid-enviroment-variable-error'

const start = async () => {
  if (!REDIS_URI || !PORT || !REDIS_PORT || !API_READ_ACCESS_TOKEN || !API_KEY || !RABBITMQ_URL || !PROXY_URL || !NODE_ENV) {
