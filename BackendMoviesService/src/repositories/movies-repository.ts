...
export class MoviesRepository implements IMoviesRepository{
    ...
    async setGenres(genres: IGenre[]): Promise<void> {
        await redis.set(TOKENS.genres, JSON.stringify(genres), {EX: 60 * 60});
    }
    ...
    async setMoviesByGenre(moviesByGenre: MoviesByGenre): Promise<void> {
        await redis.set(moviesByGenre.genre, JSON.stringify(moviesByGenre.movies), {EX: 60 * 60});
    }
    ...
    async setPopularMovies(movies: IMovie[]): Promise<void> {
        await redis.set(TOKENS.popularMovies, JSON.stringify(movies), {EX: 60 * 60});
    }
    ...
    async setAllMovies(movies: IMovie[]): Promise<void> {
        await redis.set(TOKENS.allMovies, JSON.stringify(movies), {EX: 60 * 60});;
    }
}